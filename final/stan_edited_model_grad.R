...
// compute covariance matrix as: Sigma = K * D*Q*D, where D = diag(delta) 
  ...
  for (n in 1:N) {
    ...
    for (i in 1:N) {
      for (m in 1:N) {
        Sigma_gp_all[(k * (i-1) + 1):(k*i), (k * (m-1) + 1):(k*m)] <- Sigma_gp[i,m] * (Sigma - Sigma * (locs[i,]- locs[m,])' * (locs[i,]- locs[m,]) *Sigma); 
      }
    }
    for (i in 1:(k*N)) {
      Sigma_gp_all[i,i] = Sigma_gp_all[i,i] + diag_add;
    }
}
model {
  ...
  y ~ multi_normal(mu0, Sigma_gp_all);
}
"



sim.ss_dirichlet_wishart_grad = "
data {
  int <lower=0> N;
  int <lower=0> k;
  matrix[k,k] R;
...
sim.ss_dirichlet_wishart_dimreduce_grad = "
data {
  int <lower=0> N;
  int <lower=0> k;
...
model {
  Sigma ~ inv_wishart(prior_dof, R);
  for (i in 1:N) y[(k * (i-1) + 1):(k*i)] ~ multi_normal(mu0[(k * (i-1) + 1):(k*i)], Sigma);
}
"
